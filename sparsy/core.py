...
def preprocess(dataframe: pd.DataFrame, sub_years: list[int])->tuple[int, np.ndarray, csr_matrix] | None:
...
def post_process(firms:np.ndarray, year:int, std:np.ndarray, cov_std:np.ndarray, mal:np.ndarray, cov_mal:np.ndarray, outfile:Path)-> None:
    df = pd.DataFrame(
        {
            "firm": firms,
            "year": year,
...
def process(data:pd.DataFrame, year_set:list[int], outfile:Path, IO:bool=True) -> None:
